...
public class Hitbox : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D other)
    {
        if (GetComponent<Collider2D>() is CircleCollider2D) {
            checkArchHitBox(other.transform.position, mouseDirection, range, angle);
        }
    ...
    {   
        ...
        Vector3 relativeOtherPos = otherPosition - GetComponent<Collider2D>().transform.position;
    ...
    void setMouseDirectionWithPositions(Vector3 mousePosition){
        ...
        mouseDirection = mousePosition - GetComponent<Collider2D>().transform.position;
